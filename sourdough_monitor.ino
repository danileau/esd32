...
void submitHostedInflux(unsigned long ts, float cels, float hum, float hic, long height)
{
...
void submitLogsToLoki(unsigned long ts, float cels, float hum, float hic, long height)
{
  String logMessage = String("temperature=") + cels + " humidity=" + hum + " heat_index=" + hic + 
  //" alcohol_value=" + alcohol_value + " alcohol_limit=" + alcohol_limit + 
  " height=" + height + " info";
...
void loop()
{
  ...
  Serial.print("This is a string without a newline at the end. ");
  Serial.println("This string will have a newline at the end.");
  ...
  submitHostedInflux(ts, cels, hum, hic, height);
  submitLogsToLoki(ts, cels, hum, hic, height);
